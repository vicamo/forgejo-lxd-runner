...
def serve(address: str, workers: int) -> None:
    ...
    plugin_pb2_grpc.add_BackendPluginServicer_to_server(BackendPluginService(), server)  # type: ignore[no-untyped-call]

    health_servicer = health.HealthServicer()
    health_pb2_grpc.add_HealthServicer_to_server(health_servicer, server)
    health_servicer.set("", health_pb2.HealthCheckResponse.SERVING)
    health_servicer.set("plugin.v1alpha.BackendPlugin", health_pb2.HealthCheckResponse.SERVING)
